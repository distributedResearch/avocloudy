 ...
 *
 */
public class AcaasAcoNodeBehavior extends AcaasNodeBehavior {
	...
	public AcaasAcoNodeBehavior(AbstractAcaasNodeKnowledge knwoledge,
...
//			System.out.println("Node " + nodeFound + " by ant " + (k+1) + " for app " + a.getFeature().getAppId());
	...
	public boolean antStep(Ant ant){
		...
		if (((AcaasAcoNodeBehavior)ant.getSource().getAcAgent().getBehavior()).askExecutionToNode(this.getReferringAgent().getReferringAgent(), ant.getApp())){
			...
			if (ant.getPath().size() > 0){
				...
				return ((AcaasAcoNodeBehavior)ant.getPath().get(ant.getPath().size()-1).getAcAgent().getBehavior()).stepBack(ant, this.getReferringAgent().getReferringAgent());
			}
		...
		//otherwise find another node to ask (if there are neighbors that are not in the ant path)
			...
			if (paths.size()>0){ 
				...
				IaasAgent n = this.selectNodeProbabilistically( new ArrayList<Pheromone> (paths.values()), -1, ant.getApp(), ((AcaasAcoNodeBehavior)ant.getSource().getAcAgent().getBehavior()) );
				//System.out.println("another ant step " + n);
				return ((AcaasAcoNodeBehavior)n.getAcAgent().getBehavior()).antStep(ant);
			}  
		}
	...
	public boolean stepBack(Ant ant, IaasAgent from){
		...
		((AcaasAcoNodeKnowledge)this.getKnowledge()).getPheromonePath().get(from).depositPheromone(((AcaasAcoNodeBehavior)ant.getSource().getAcAgent().getBehavior()).policy, p);
		...
		if (ant.getPath().size() > 1){
			...
			return ((AcaasAcoNodeBehavior)ant.getPath().get(ant.getPath().size()-1).getAcAgent().getBehavior()).stepBack(ant, source);
		}
		else {
	 ...
	 * @return
	 */
	private IaasAgent selectNodeProbabilistically(ArrayList<Pheromone> nodes, int color, AppAgent app, AcaasAcoNodeBehavior antOwnerBehavior){
...
//	}
	 ...
	 * @param nodes
	 */
	private void repartProbToOther(ArrayList<Pheromone> nodes, int color, AppAgent app, AcaasAcoNodeBehavior antOwnerBehavior){
...
//		this.resetColoredTemperature();
	...
	public void coloredAntStep(ColoredAnt ant){
		...
		if ( ant.getTtlValue()>0 ){
			...
			if (paths.size()>0){ 
				...
				IaasAgent n = this.selectNodeProbabilistically( new ArrayList<Pheromone> (paths.values()), ant.getAntColor(), null, ((AcaasAcoNodeBehavior)ant.getSource().getAcAgent().getBehavior()) );
				
				//System.out.println("coloredstep through " + n);
				((AcaasAcoNodeBehavior)n.getAcAgent().getBehavior()).coloredAntStep(ant);
				return;
			} 
		...
		//else {
			...
			if (stepIdStart > 0){
				...
				((AcaasAcoNodeBehavior)ant.getPath().get(stepIdStart-1).getAcAgent().getBehavior()).coloredAntStepBack(ant, this.getReferringAgent().getReferringAgent(), stepIdStart);
	...
	public void coloredAntStepBack(ColoredAnt ant, IaasAgent from, int stepIdStart){
		...
		if (stepBackIdStart > 0){
			...
			((AcaasAcoNodeBehavior)ant.getPath().get(stepBackIdStart-1).getAcAgent().getBehavior()).coloredAntStepBack(ant, this.getReferringAgent().getReferringAgent(), stepBackIdStart);
			return;
		}
	...
	public void depositColoredPheromone(ColoredAnt ant, IaasAgent from, int stepIdStart){
		...
		//int step = ant.getPath().size()-1;
		((AcaasAcoNodeKnowledge)this.getKnowledge()).getPheromone(from).depositColoredPheromone(((AcaasAcoNodeBehavior)ant.getSource().getAcAgent().getBehavior()).policy, /*ant.getBestColorFound(),*/ stepIdStart, ant.getAntColor(), ant);
	}
